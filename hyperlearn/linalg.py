...
def svd(X, U_decision = False, overwrite = False):
	"""
	[Added 19/11/2018]
	...
	svd_flip(U, VT, U_decision = U_decision)
...
def pinv(X, alpha = None, overwrite = False):
	...
	Algorithm to guarantee convergence. Allows Ridge Regularization - default 1e-8.
...
def eigh(X, alpha = None, svd = False, overwrite = False):
	"""
	[Added 21/11/2018]
	...
	if syevd:
		decomp = lapack("syevd")
		L, V = do_until_success(decomp, add_jitter, size, overwrite, alpha, 
